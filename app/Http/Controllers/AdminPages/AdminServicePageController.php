...
class AdminServicePageController extends Controller
{
    ...
    public function storeServicePageFormEdits(ServicePageFormRequest $request)
    {
        ...
        $dataForDatabaseTable = array(
            ...
            //////////////////////////////////////
            'divider_title' => filter_var($request->input('title'), FILTER_SANITIZE_SPECIAL_CHARS, FILTER_FLAG_ENCODE_LOW),
            'divider_text' => filter_var($request->input('caption'), FILTER_SANITIZE_SPECIAL_CHARS,           FILTER_FLAG_ENCODE_LOW),
            'updated_at' => now()
        );
        ...
        /*******************************************************/
        session()->flash('info', 'Good job! The Service page info has been updated, successfully.');
